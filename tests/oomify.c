...
static int oomify_spawn(char *argv[], const struct oomctl *ctl, struct oomstat *stats, int *wstatus) {
	...
	if (bfs_spawn_addclose(&ctx, ctlpipe[1]) != 0) {
		error = errno;
		goto out_ctx;
	}
	if (ctlpipe[0] != OOMCTL_FILENO) {
		if (bfs_spawn_adddup2(&ctx, ctlpipe[0], OOMCTL_FILENO) != 0) {
			error = errno;
			goto out_ctx;
		}
		if (bfs_spawn_addclose(&ctx, ctlpipe[0]) != 0) {
			error = errno;
			goto out_ctx;
		}
	}
	...
	if (bfs_spawn_addclose(&ctx, statpipe[0]) != 0) {
		goto out_ctx;
	}
	if (statpipe[1] != OOMSTAT_FILENO) {
		if (bfs_spawn_adddup2(&ctx, statpipe[1], OOMSTAT_FILENO) != 0) {
			error = errno;
			goto out_ctx;
		}
		if (bfs_spawn_addclose(&ctx, statpipe[1]) != 0) {
			error = errno;
			goto out_ctx;
		}
	}
	...
	envp[envc - 1] = "LD_PRELOAD=/home/tavianator/code/bfs/tests/liboomify.so";
	envp[envc] = NULL;

	pid_t pid = bfs_spawn(argv[0], &ctx, argv, envp);
	if (pid < 0) {
		error = errno;
		goto out_envp;
	}
...
int main(int argc, char *argv[]) {
	...
	size_t min, max;
	if (dry_run || ctl.inject_at == SIZE_MAX) {
